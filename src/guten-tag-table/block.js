/**
 * BLOCK: tag-groups-premium-cloud-table
 *
 *
 * @package     Tag Groups
...
const helpFeature = 'table-tag-cloud/table-tag-cloud-gutenberg-block/';

class tagGroupsPremiumTableCloudParameters extends Component {
  ...
  render() {
    ...
    renderAttributes.table_id = this.state.uniqueId;
    ...
    delete renderAttributes.td_class;
    delete renderAttributes.th_class;
    ...
    return [
        ...
        <div className='chatty-mango-inspector-control'>
          ...
          <PanelBody title={__('Groups')} initialOpen={false}>
            <TagGroupsHelp
              ...
              feature={helpFeature}
              siteLang={siteLang}
              topic='hide_empty_columns'
            />
            <ToggleControl
              label={__('Hide empty columns')}
              checked={hide_empty_columns}
              onChange={() => toggleOptionHideEmptyColumns(this)}
            />
            <TagGroupsHelp
              url={helpUrl}
              product={helpProduct}
          ...
          <PanelBody title={__('Advanced Styling')} initialOpen={false}>
            <TagGroupsHelp
              ...
              feature={helpFeature}
              siteLang={siteLang}
              topic='responsive_breakpoint'
            />
            <RangeControl
              label={
                __('Responsive breakpoint') +
                ' ' +
                (responsive_breakpoint === 0
                  ? '(' + __('responsiveness off') + ')'
                  : '')
              }
              value={
                responsive_breakpoint !== undefined
                  ? Number(responsive_breakpoint)
                  : 800
              }
              onChange={(responsive_breakpoint) => {
                setAttributes({ responsive_breakpoint });
              }}
              min={0}
              max={2048}
            />
            <div>
              <TagGroupsHelp
                ...
                topic='table_id'
              />
              <label htmlFor='tg_input_table_id'>{'<table id="...">'}</label>
            </div>
            <PlainText
              id='tg_input_table_id'
              className='input-control'
              value={table_id ? table_id : ''}
              placeholder={__('Write here or leave empty.')}
              onChange={(table_id) => setAttributes({ table_id })}
            />
            <div>
              <TagGroupsHelp
                url={helpUrl}
                ...
                topic='table_class'
              />
              <label htmlFor='tg_input_table_class'>{__('table class')}</label>
            </div>
            <PlainText
              id='tg_input_table_class'
              className='input-control'
              value={table_class ? table_class : ''}
              placeholder={__('Write here or leave empty.')}
              onChange={(table_class) => setAttributes({ table_class })}
            />
            <div>
              <TagGroupsHelp
                url={helpUrl}
                ...
                topic='th_class'
              />
              <label htmlFor='tg_input_th_class'>{'th class'}</label>
            </div>
            <PlainText
              id='tg_input_th_class'
              className='input-control'
              value={th_class ? th_class : ''}
              placeholder={__('Write here or leave empty.')}
              onChange={(th_class) => setAttributes({ th_class })}
            />
            <div>
              <TagGroupsHelp
                url={helpUrl}
                ...
                topic='td_class'
              />
              <label htmlFor='tg_input_td_class'>{__('td class')}</label>
            </div>
            <PlainText
              id='tg_input_td_class'
              className='input-control'
              value={td_class ? td_class : ''}
              placeholder={__('Write here or leave empty.')}
              onChange={(td_class) => setAttributes({ td_class })}
            />
            {tags_post_id !== -1 && (
              <div>
                <div>
      ...
      <div className='chatty-mango-overflow-auto'>
        {!!cover && (
          <Fragment>
        ...
        {!cover && serverSideRender && (
          <TagGroupsServerSideRender
            block='chatty-mango/tag-groups-premium-cloud-table'
...
var cmTagGroupsTableBlock = registerBlockType(
  'chatty-mango/tag-groups-premium-cloud-table',
  {
    title: __('Table Tag Cloud'),
    icon: 'tagcloud', // Block icon from Dashicons → https://developer.wordpress.org/resource/dashicons/.
    category: 'chatty-mango',
    description: __('Show your tags in groups in a table.'),
    keywords: [__('table'), __('tag cloud'), 'Chatty Mango'],
    html: false,
    transforms: {
      to: [
        {
          ...
          transform: function (attributes) {
            ...
            let text = '[tag_groups_table ' + parameters.join(' ') + ']';
     ...
     */
    edit: tagGroupsPremiumTableCloudParameters,
